...
// solution
// circles intersect, if the distance between the centers
// is smaller than (or equal to) the sum of the radii
template <class FLOAT, size_t N>
bool Sphere<FLOAT,N>::intersects(Sphere<FLOAT, N> sphere) const {
  Vector<FLOAT, N> distance_vector = sphere.center - center;
...
bool Sphere<FLOAT,N>::inside(const Vector<FLOAT, N> p) const {
        Vector<FLOAT, N> distance_vector = p - center;
        FLOAT distance = distance_vector.length();
